...
class GraphsFragment : Fragment() {
    ...
    private fun getalldata(){
        ...
        db.collection("planting").whereEqualTo("estateId",estateId).limit(1).get().addOnSuccessListener {
                documents ->
    ...
    private fun getStateData(){
         ...
         var transformidad = 1.0
         var transformidad_f2 = 2520.0
        ...
        db.collection("planting").whereEqualTo("estateId",estateId).limit(1).get().addOnSuccessListener {
         ...
         var anio_analizado_c1 = 1.0 // le asigne un valor para que no genere error



        db.collection("harvest").whereEqualTo("estateId",estateId).limit(1).get().addOnSuccessListener {
                documents ->
            if (!documents.isEmpty) {
                val document = documents.documents[0]
        ...
        //drying
         ...
         var transformidad_s6 = 26500000000000.0
         var transformidad_s7 = 22500000000000.0
         var transformidad_s8 = 111111.0
         var transformidad_s9 = 26500000000000.0
         var transformidad_s30 = 26500000000000.0
         var v_dolar2014 = 2000.0


        try {

            db.collection("drying").whereEqualTo("estateId",estateId).limit(1).get().addOnSuccessListener {
                    documents ->
                if (!documents.isEmpty) {
                    val document = documents.documents[0]
                    plantingId = document.id
                    ...
                    db.collection("drying").document(document.id).get()
                        .addOnSuccessListener { documentSnapshot ->
                            if (documentSnapshot.exists()) {
        ...
        db.collection("beaten").whereEqualTo("estateId",estateId).limit(1).get().addOnSuccessListener {
                documents ->
            if (!documents.isEmpty) {
                val document = documents.documents[0]
                plantingId = document.id
                db.collection("beaten").document(document.id).get()
                    .addOnSuccessListener { documentSnapshot ->
                        if (documentSnapshot.exists()) {
